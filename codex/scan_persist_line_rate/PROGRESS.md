...
notes:
  ...
  - "2026-01-26: TUI mutation gating fixes + SQLite filter + millis timestamps + query table browser schema quoting + bulk-tag save-as-rule + rule IDs + open_existing guard + cancel docs + discover UI test update. Ran cargo test -p casparian (pass). Ran cargo bench -p casparian --bench scanner_perf; walk_parallel 15.005-20.143 ms; full_scan batch_size=512 137.06-141.28 ms, batch_size=2048 164.68-275.32 ms, batch_size=10000 178.31-219.59 ms; rescan batch_size=512 178.13-213.00 ms, batch_size=2048 162.87-174.64 ms, batch_size=10000 158.88-180.22 ms; db_write batch_size=256 157.55-165.93 ms, batch_size=1024 112.76-127.68 ms, batch_size=4096 104.35-126.36 ms, batch_size=10000 128.45-158.46 ms."
  - "2026-01-28: Non-scan work: sentinel dispatch lease refactor (sqlite executor + catalog thread usage), control-plane async replies, dispatch prep in DB thread, and lease-based conclude handling. Ran cargo check -p casparian_sentinel (pass). Did not run cargo test -p casparian or scanner_perf bench."
---
...
# Next step
- Review scanner_perf regressions from 2026-01-26 run and decide whether to proceed with optional M4 (arena + packed records) or investigate benchmark noise.
