...
class WorkerContext:
    ...
    def register_topic(self, topic: str, default_uri: str = None) -> int:
        ...
        sink = SinkFactory.create(
            uri, self.sql_engine, self.parquet_root, job_id=self.job_id
        )
    ...
    def _handle_inspection(self, handle: int, data: Any):
        ...
        if topic_name not in self.captured_schemas:
            ...
            if hasattr(data, "schema"):  # Arrow
                schema_info = {
                    n: str(t) for n, t in zip(data.schema.names, data.schema.types)
                }
    ...
    def _validate_schema(self, data, required_schema):
        ...
        if missing:
            raise SchemaViolationError(
                f"Strict Schema Violation: Missing columns {missing}"
            )
