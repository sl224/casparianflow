...
class SqliteSink:
    ...
    def promote(self):
        with self.engine.begin() as conn:
            ...
            if not exists:
                conn.execute(text(f"ALTER TABLE {self.staging_table} RENAME TO {self.table_name}"))
            else:
                conn.execute(text(f"INSERT INTO {self.table_name} SELECT * FROM {self.staging_table}"))
    ...
    def write(self, data: Any):
        ...
        if isinstance(data, pd.DataFrame):
            if HAS_ARROW:
                ...
                data.to_parquet(self.staging_path, compression=self.compression, index=False)
                return
        ...
        if table:
            if self.writer is None:
                self.writer = pq.ParquetWriter(self.staging_path, table.schema, compression=self.compression)
...
class SinkFactory:
    @staticmethod
    def create(uri: str, sql_engine: Engine, parquet_root: Path, job_id: int = 0) -> DataSink:
        parsed = urlparse(uri)
