...
class ParallelFileScanner:
    ...
    def walk(
        self, root_path: Path, filter_func: Callable[[os.DirEntry], bool]
    ) -> Iterator[Path]:
        """
        ...
        walker_thread = threading.Thread(
            target=self._run_walker, args=(root_path, filter_func)
        )
        walker_thread.start()
            ...
            while active_futures:
                ...
                for f in done_futures:
                    ...
                    try:
                        ...
                        for d in subdirs:
                            active_futures.add(
                                executor.submit(self._scan_dir, d, filter_func)
                            )
