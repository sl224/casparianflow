...
def unpack_header(data: bytes) -> Tuple[OpCode, int, int, ContentType, bool]:
    ...
    version, op_raw, flags, meta_len, job_id = struct.unpack(HEADER_FORMAT, data[:HEADER_SIZE])
...
def msg_hello(capabilities: list) -> list:
    ...
    return [
        pack_header(OpCode.HELLO, 0, len(payload), ContentType.JSON),
        payload
    ]
...
def msg_exec(job_id: int, plugin_name: str, file_path: str) -> list:
    ...
    return [
        pack_header(OpCode.EXEC, job_id, len(payload), ContentType.JSON),
        payload
    ]
...
def msg_err(job_id: int, message: str) -> list:
    ...
    return [
        pack_header(OpCode.ERR, job_id, len(payload), ContentType.UTF8),
        payload
    ]