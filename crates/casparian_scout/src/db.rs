...
use crate::types::{DbStats, FileStatus, ScannedFile, Source, SourceType, TaggingRule, UpsertResult};
...
impl Database {
    ...
    pub async fn set_manual_plugin(&self, id: i64, plugin_name: &str) -> Result<()> {
        sqlx::query("UPDATE scout_files SET manual_plugin = ? WHERE id = ?")
            .bind(plugin_name)
            .bind(id)
            .execute(&self.pool)
            .await?;
    ...
    /// Clear all manual overrides for a file (resets to auto)
    pub async fn clear_manual_overrides(&self, id: i64) -> Result<()> {
        sqlx::query(
            "UPDATE scout_files SET tag_source = NULL, rule_id = NULL, manual_plugin = NULL, tag = NULL, status = 'pending' WHERE id = ?",
        )
        .bind(id)
        .execute(&self.pool)
        .await?;
    ...
    pub async fn list_manual_files(&self, source_id: &str, limit: usize) -> Result<Vec<ScannedFile>> {
        let rows = sqlx::query(
            r#"
            SELECT id, source_id, path, rel_path, size, mtime, content_hash, status, tag, tag_source, rule_id, manual_plugin, error,
                   first_seen_at, last_seen_at, processed_at, sentinel_job_id
            FROM scout_files
            WHERE source_id = ? AND (tag_source = 'manual' OR manual_plugin IS NOT NULL)
            ORDER BY mtime DESC
            LIMIT ?
            "#,
        )
    ...
    pub async fn list_all_failed_files(&self, limit: usize) -> Result<Vec<ScannedFile>> {
        self.list_files_by_status(FileStatus::Failed, limit).await
    }

    /// List failed files for a specific source
    pub async fn list_failed_files_for_source(&self, source_id: &str, limit: usize) -> Result<Vec<ScannedFile>> {
        self.list_files_by_source_and_status(source_id, FileStatus::Failed, limit).await
    }

    /// Reset failed files to pending status
    pub async fn retry_failed_files(&self, source_id: &str) -> Result<u64> {
        let result = sqlx::query(
            "UPDATE scout_files SET status = 'pending', error = NULL, sentinel_job_id = NULL WHERE source_id = ? AND status = 'failed'",
        )
        .bind(source_id)
        .execute(&self.pool)
        .await?;
    ...
    /// List files that need status sync from Sentinel.
    /// Returns (file_id, sentinel_job_id) for files with:
    /// - status IN ('queued', 'processing')
    /// - sentinel_job_id IS NOT NULL
    pub fn list_files_needing_sync(&self) -> Result<Vec<(i64, i64)>> {
        // Use blocking query since scout database is synchronous in Tauri context
        ...
        if let Ok(handle) = rt {
            ...
            let rt = tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .map_err(|e| crate::error::ScoutError::InvalidState(format!("Failed to create runtime: {}", e)))?;
            rt.block_on(async {
                let rows: Vec<(i64, i64)> = sqlx::query_as(
                    r#"
                    SELECT id, sentinel_job_id
    ...
    /// Update file status based on Sentinel job completion.
    /// Called by sync_scout_file_statuses to update file status from job status.
    pub fn update_file_status_from_job(&self, id: i64, status: &str, error: Option<&str>) -> Result<()> {
        // Use blocking query since scout database is synchronous in Tauri context
        let rt = tokio::runtime::Handle::try_current();
        let status_owned = status.to_string();
        let error_owned = error.map(|s| s.to_string());

        let update_fn = |pool: SqlitePool, status: String, error: Option<String>, id: i64| async move {
            if status == "processed" {
                sqlx::query("UPDATE scout_files SET status = ?, error = ?, processed_at = ? WHERE id = ?")
                    .bind(&status)
                    .bind(&error)
        ...
        if let Ok(handle) = rt {
            handle.block_on(update_fn(self.pool.clone(), status_owned, error_owned, id))
            ...
            let rt = tokio::runtime::Builder::new_current_thread()
                ...
                .build()
                .map_err(|e| crate::error::ScoutError::InvalidState(format!("Failed to create runtime: {}", e)))?;
            rt.block_on(update_fn(self.pool.clone(), status_owned, error_owned, id))
                .map_err(crate::error::ScoutError::from)
        }
    }
    ...
    pub async fn get_tag_stats(&self, source_id: &str) -> Result<Vec<(String, u64, u64, u64)>> {
        ...
        Ok(rows
            .into_iter()
            .map(|(tag, total, processed, failed)| {
                (tag, total as u64, processed as u64, failed as u64)
            })
            .collect())
    }
    ...
    pub async fn delete_setting(&self, key: &str) -> Result<bool> {
        let result = sqlx::query("DELETE FROM scout_settings WHERE key = ?")
            .bind(key)
            .execute(&self.pool)
            .await?;
        Ok(result.rows_affected() > 0)
...
mod tests {
    ...
    async fn test_untagged_files() {
        ...
        // All should be untagged
        let untagged = db.list_untagged_files("src-1", 10).await.unwrap();
        assert_eq!(untagged.len(), 5);

        // Tag some
        db.tag_file(untagged[0].id.unwrap(), "csv_data").await.unwrap();
        db.tag_file(untagged[1].id.unwrap(), "csv_data").await.unwrap();

        // Should now have 3 untagged
        let untagged = db.list_untagged_files("src-1", 10).await.unwrap();
    ...
    async fn test_stats_with_tags() {
        ...
        for i in 0..10 {
            ...
            if i < 3 {
                ...
                db.tag_file(result.id, "json_data").await.unwrap();
            }
            // Rest remain untagged
        }

        let stats = db.get_stats().await.unwrap();
        assert_eq!(stats.total_files, 10);
        assert_eq!(stats.files_pending, 5); // Untagged
        assert_eq!(stats.files_tagged, 5);  // Tagged
    }

    #[tokio::test]
    async fn test_parser_lab_tables_exist() {
        // Test that Parser Lab tables are created by the schema
        let db = create_test_db().await;
        let pool = db.pool();

        // Should be able to insert into parser_lab_parsers
        ...
        let result = sqlx::query(
            r#"INSERT INTO parser_lab_parsers
               ...
               VALUES ('test-parser', 'Test Parser', '*.csv', 'glob', 'pending', 'parquet', 0, ?, ?)"#,
        )
        .bind(now)
        .bind(now)
        .execute(pool)
        .await;
        assert!(result.is_ok(), "Should be able to insert into parser_lab_parsers");

        // Should be able to read it back
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM parser_lab_parsers WHERE id = 'test-parser'")
            .fetch_one(pool)
            .await
            .unwrap();
        assert_eq!(count, 1);

        // Should be able to insert test file (FK to parser)
        let result = sqlx::query(
            r#"INSERT INTO parser_lab_test_files
               (id, parser_id, file_path, file_name, file_size, created_at)
               VALUES ('file-1', 'test-parser', '/data/test.csv', 'test.csv', 1024, ?)"#,
        )
        .bind(now)
        .execute(pool)
        .await;
        assert!(result.is_ok(), "Should be able to insert test file with FK");

        // Test ON DELETE CASCADE - deleting parser should delete test files
        sqlx::query("DELETE FROM parser_lab_parsers WHERE id = 'test-parser'")
            .execute(pool)
            .await
            .unwrap();

        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM parser_lab_test_files WHERE parser_id = 'test-parser'")
            .fetch_one(pool)
            .await
            .unwrap();
        assert_eq!(count, 0, "ON DELETE CASCADE should have deleted test files");
    }

    #[tokio::test]
    async fn test_parser_lab_update_roundtrip() {
        // Test full Parser Lab roundtrip: create, update, verify
        let db = create_test_db().await;
        let pool = db.pool();
        ...
        sqlx::query(
            r#"INSERT INTO parser_lab_parsers
               (id, name, file_pattern, validation_status, sink_type, is_sample, created_at, updated_at)
               VALUES ('p1', 'CSV Parser', '*.csv', 'pending', 'parquet', 0, ?, ?)"#,
        )
        .bind(now)
        .bind(now)
        .execute(pool)
        ...
        // Update with source code and validation
        let source_code = r#"
import polars as pl
...
"#;
        ...
        // Verify the update
        let (status, code, output): (String, Option<String>, Option<String>) = sqlx::query_as(
            "SELECT validation_status, source_code, validation_output FROM parser_lab_parsers WHERE id = 'p1'"
        )
        .fetch_one(pool)
        .await
        .unwrap();
        ...
        assert!(code.is_some());
        assert!(code.unwrap().contains("import polars"));
        assert_eq!(output, Some("All tests passed".to_string()));
    }
}
