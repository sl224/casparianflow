...
pub struct ProcessingJob {
    ...
    pub dispatch_ack_at: Option<i64>,
...
impl ProcessingJob {
    ...
    pub fn from_row(row: &UnifiedDbRow) -> Result<Self, BackendError> {
        ...
        Ok(Self {
            ...
            lease_token: row.get_by_name("lease_token")?,
            lease_owner: row.get_by_name("lease_owner")?,
            lease_expires_at: row.get_by_name("lease_expires_at")?,
            dispatch_ack_at: row.get_by_name("dispatch_ack_at")?,
...
impl DeadLetterJob {
    pub fn from_row(row: &UnifiedDbRow) -> Result<Self, BackendError> {
        Ok(Self {
            ...
            file_id: row.get_by_name("file_id")?,
            input_file: row.get_by_name("input_file")?,
            plugin_name: row.get_by_name("plugin_name")?,
            error_message: row.get_by_name("error_message")?,
            retry_count: row.get_by_name("retry_count")?,
